...
local Cfg = {
    ...
    GUI_VISIBLE = true,
    CAM_NOCLIP = false,
    TEAM_COLOR = Color3.fromRGB(0, 255, 0),
}
...
local Theme = {
    ...
    accentDim  = Color3.fromRGB(0, 180, 45),
...
do
    ...
    function loadEvent()
        ...
        if Ref.Event then print("[Uriel] event: "..Ref.Event:GetFullName()) else task.delay(1.5, loadEvent) end
    end
    loadEvent()
end
...
-- ============ CONFIG SAVE/LOAD ============
do
    local CFG_DIR = "UrielPainel"
    local CFG_PATH = "UrielPainel/Config.json"
    ...
    local saveKeysNum = {
        ...
        "LOS_X_SPREAD","LOS_Y_SPREAD","AFK_SPEED",
    }
    local saveKeysStr = {
    ...
    function saveConfig()
        local ok, err = pcall(function()
            ...
            print("[Uriel] config saved ✓")
        end)
        if not ok then print("[Uriel] config save FAILED: "..tostring(err)) end
    end

    function loadConfig()
        local ok, err = pcall(function()
            if not isfile(CFG_PATH) then print("[Uriel] no config file, using defaults"); return end
            local raw = readfile(CFG_PATH)
            if not raw or #raw == 0 then print("[Uriel] config file empty, skipping"); return end
            local data = Svc.Http:JSONDecode(raw)
            if type(data) ~= "table" then print("[Uriel] config parse error, skipping"); return end
            local loaded = 0
            ...
            print("[Uriel] config loaded ✓ ("..loaded.." keys)")
        end)
        if not ok then print("[Uriel] config load FAILED: "..tostring(err)) end
    end
...
function fireReload(reason)
    ...
    if ok then St.shotCounter=0; print("[Uriel] reload | "..tostring(reason)) end; return ok
end
...
function setRunning(on)
    if on then
        ...
        if GUI.statusLabel then GUI.statusLabel.Text="ACTIVE"; GUI.statusLabel.TextColor3=Theme.green end
...
function toggleSword()
    ...
    print("[Uriel] "..(Cfg.MINECRAFT_SWORD and "Sword ON" or "Sword OFF"))
end
...
function toggleFP()
    ...
    if Cfg.FP_ON then
        ...
        print("[Uriel] FP ON | char + GlockLiver hidden")
    else
        stopFPCamera()
        setLocalTransp(myChar(), 0)
        setGlockLiverVisible(true)
        print("[Uriel] FP OFF | char + GlockLiver visible")
    end
...
local function ensureESP(plr)
    ...
    local d={
        ...
        box=mkDraw("Square",{Thickness=1,Color=Color3.fromRGB(0,255,65),Filled=false}),
        corners={},
        ...
        snap=mkDraw("Line",{Thickness=1,Color=Color3.fromRGB(0,255,65),Transparency=0.55}),
        charRef=nil,
    }
    for i=1,8 do d.corners[i]=mkDraw("Line",{Thickness=2,Color=Color3.fromRGB(0,255,65)}) end
...
local function updateESP()
    ...
    for _,plr in ipairs(Svc.Players:GetPlayers()) do
        ...
        else col=Color3.fromRGB(0,255,65) end
        pcall(function()
...
pcall(function()
    ...
    local o=p:FindFirstChild("UrielPainel"); if o then o:Destroy() end
end)

local gui=Instance.new("ScreenGui")
gui.Name="UrielPainel"; gui.ResetOnSpawn=false; gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling
...
glow.Color=Theme.green; glow.Thickness=2; glow.Transparency=0.3
...
titleText.TextColor3=Theme.green; titleText.TextXAlignment=Enum.TextXAlignment.Left
titleText.Text="URIEL PAINEL RBLX"
...
verText.Text="v1.0 Made By Uriel"
...
local function makeSlider(parent, name, min, max, default, callback, order, step)
    ...
    valLbl.TextColor3=Theme.green; valLbl.TextXAlignment=Enum.TextXAlignment.Right; valLbl.Text=tostring(default)
...
local function makeCycleBtn(parent, name, options, default, callback, order)
    ...
    btn.TextColor3=Theme.green; btn.Text=options[default] or "?"
        ...
        Ref.rebindConn=Svc.UIS.InputBegan:Connect(function(input,gp)
            ...
            if input.UserInputType==Enum.UserInputType.Keyboard then
                ...
                if newKey==Enum.KeyCode.Escape then
                    bindBtn.Text=keyName(Binds[actionName])
                    bindBtn.TextColor3=Theme.green; bs.Color=Theme.border
                    St.rebinding=nil
                ...
                St.rebinding=nil
                pcall(function() Ref.rebindConn:Disconnect() end); Ref.rebindConn=nil
                print("[Uriel] "..actionName.." -> "..keyName(newKey))
            end
        end)
...
for i,tabName in ipairs(tabs) do
    ...
    page.ScrollBarThickness=3; page.ScrollBarImageColor3=Theme.green
...
do
    ...
    makeToggle(pg,"AntiWallBang",Cfg.ANTI_WALLBANG,function(v) Cfg.ANTI_WALLBANG=v end,2)
    makeToggle(pg,"Skip Teammates",Cfg.SKIP_TEAMMATES,function(v) Cfg.SKIP_TEAMMATES=v end,3)
    makeToggle(pg,"Auto Reload",Cfg.AUTO_RELOAD,function(v) Cfg.AUTO_RELOAD=v end,4)
    makeToggle(pg,"Skip ForceField",Cfg.SKIP_FORCEFIELD,function(v) Cfg.SKIP_FORCEFIELD=v end,5)
    makeToggle(pg,"Only Nearest",Cfg.ONLY_NEAREST,function(v) Cfg.ONLY_NEAREST=v end,6)
    makeToggle(pg,"FOV Check",Cfg.AIM_FOV_ON,function(v) Cfg.AIM_FOV_ON=v end,7)
    makeSlider(pg,"FOV",30,360,Cfg.AIM_FOV,function(v) Cfg.AIM_FOV=v end,8,5)
    makeSlider(pg,"Max Distance",50,600,Cfg.MAX_DIST,function(v) Cfg.MAX_DIST=v end,9,10)
    makeSlider(pg,"Ammo",1,50,Cfg.AMMO_VALUE,function(v) Cfg.AMMO_VALUE=v end,10,1)
    makeSlider(pg,"Fire Delay (ms)",1,300,math.floor(Cfg.SHOT_DELAY*1000),function(v) Cfg.SHOT_DELAY=v/1000 end,11,1)
    makeSlider(pg,"AWB Delay (ms)",0,500,math.floor(Cfg.AWB_EXTRA_DELAY*1000),function(v) Cfg.AWB_EXTRA_DELAY=v/1000 end,12,10)
end
...
do
    ...
    swBtn.TextColor3=Theme.green; swBtn.Text="TEST SWING"
...
do
    ...
    svBtn.TextColor3=Theme.green; svBtn.Text="SAVE CONFIG"
    ...
    dBtn.MouseButton1Click:Connect(function()
        if getgenv().UrielPainel and getgenv().UrielPainel.destroy then getgenv().UrielPainel.destroy() end
    end)
end
...
getgenv().UrielPainel={
    ...
    destroy=function()
        ...
        stopEmoteAA(); stopBhop(); stopAA(); stopESP(); stopCamNoclip()
        ...
        getgenv().UrielPainel=nil
    end,
}

print("[Uriel Painel RBLX] v1.0 Made By Uriel | \\ to toggle GUI")
print("[Uriel] ready | rebind keys in Misc tab")
